# Liner Notes — MVP Product Requirements Document
...
**Media attachments** (one or more per idea):

- Audio recording (captured in-app from mic, or imported file)  
...
**Production journal:** Running freeform rich-text notes with image embed support. This is the ongoing narrative of how the song is being made. Per-instrument writeups (gear used, signal chain, presets saved, how the sound evolved), mix decisions, creative pivots, session notes. This content *becomes* the liner notes.
...
- Record audio from device microphone with one tap/click  
...
- Detect and connect to MIDI controllers (USB or Bluetooth)  
...
- Define custom sections (user-named, reorderable)  
...
- Production journal (rich text with image embeds, organized by topic/instrument)  
...
- **Capture interfaces are abstracted.** The web app uses Web MIDI API and MediaRecorder API behind a clean interface. When a native mobile app arrives, it implements the same interface via CoreMIDI (iOS) / Android MIDI API and native audio recording. The rest of the app doesn't care.  
- **All processing is client-side.** Audio-to-MIDI runs in the browser via the @spotify/basic-pitch TypeScript library. Synth playback runs in the browser via Tone.js and soundfont-player. No server dependencies to recreate when mobile arrives — the same libraries can be bundled into a React Native app or replaced with native equivalents.

**Key technical stack (see Tech Architecture doc for full detail):**

- React \+ Vite SPA, TypeScript, Tailwind CSS \+ shadcn/ui  
- Web MIDI API for MIDI controller input (Chrome, Edge, Firefox 108+, Opera — not Safari/iOS)  
- Tone.js (v15.1.22) \+ soundfont-player for synth engine and instrument playback (\~10 patches: piano, electric piano, bass, synth bass, brass, strings, synth lead, synth pad, organ, mallet)  
...
- Dexie.js (IndexedDB wrapper) for all local data and media storage  
- @tonejs/midi for MIDI file import/export  
- File System Access API \+ JSZip for export to filesystem
...
- Native iOS/Android apps for mobile capture with CoreMIDI and native audio
...
2. **Audio-to-MIDI accuracy expectations.** Basic Pitch is good but not perfect, especially for polyphonic content. Need to set user expectations and provide easy correction workflows. May want to let users manually adjust extracted MIDI via the note picker.  
...
4. **Note/chord picker UX.** How much musical structure should the picker expose? Just individual notes with octave selection? Common chord presets (major, minor, 7th, sus)? Custom chord building? The goal is "fast enough to capture what you just played" without becoming a theory tool. Needs prototyping.  
     
5. **Mobile roadmap timing.** MVP is desktop-only, but when does mobile become critical? Post-validation with early users? After the first handful of paying customers? Mobile is where the highest-friction capture problem lives (couch \+ guitar \+ phone), so it can't wait too long.  
     
6. **Browser storage limits.** IndexedDB storage is generous (typically several GB) but not unlimited. For a prolific songwriter with lots of audio recordings, this could become a constraint. Need to monitor usage and consider an export/archive workflow or cloud backup before storage becomes an issue.
