\# Nootbuk — Wireframes & User Flow

Draft v0.3 — August 2026

Changes from v0.2: Product renamed from "Liner Notes" to "Nootbuk". Updated stack references (Vite 8, Tailwind v4 CSS-first, shadcn new-york style, smplr, @dnd-kit, Tiptap 3).
...
\- \*\*React 19 \+ Vite 8 SPA\*\* — single-page app, client-side routing  
\- \*\*Tailwind CSS v4 \+ shadcn/ui\*\* — Tailwind v4 uses CSS-first config via @theme (no tailwind.config.ts). shadcn uses new-york style with OKLCH colors. Use shadcn component primitives (Button, Dialog, Sheet, Card, DropdownMenu, Tabs, etc.) with Tailwind utility classes  
...
│   ├── Sections Panel (user-defined, reorderable via @dnd-kit)  
│   │   └── Ideas within each section (reorderable, draggable between sections)  
│   ├── Song Lyrics View  
│   ├── Production Journal (Tiptap 3\)  
│   ├── References  
...
│   ├── Global production notes (Tiptap 3\)  
│   └── Reference material
...
\*\*Entry point:\*\* The Capture button on the Home screen, or a global keyboard shortcut. Also accessible via "Add Idea" within a song section (pre-selects that section).
...
\*\*Critical UX principle: an idea can contain multiple types of content.\*\* A single idea might be a MIDI recording \+ a lyric line \+ a photo of amp settings. The capture UI must NOT force the user to pick one mode. Instead, all capture tools are available simultaneously and content stacks up within the same idea.

\*\*Content:\*\* The capture area is a vertical stack. Each content block is added via action buttons at the top (or a toolbar). The user can add as many blocks as they want in any combination:

\*\*Action buttons / toolbar:\*\*

\- 🎤 Record Audio — adds an audio recording block. Large Record button, tap to start/stop. Waveform visualizes during and after recording (captured as lossless WAV via AudioWorklet). Playback controls. "Extract MIDI" button appears after recording.  
\- 🎹 Record MIDI — adds a MIDI recording block. Shows connected MIDI devices (or "No MIDI device detected" / "MIDI not supported in this browser" if Web MIDI API unavailable). Device selector, patch selector (\~10 sounds: Piano, E. Piano, Bass, Synth Bass, Brass, Strings, Synth Lead, Synth Pad, Organ, Mallet — powered by Tone.js \+ smplr), metronome toggle with BPM. Record button. After recording, shows note list with playback.  
\- 🎵 Note Picker — adds a manual note/chord entry block. Visual keyboard or note grid. Select note \+ octave, add to sequence. Chord presets available. Horizontal sequence with play button.  
\- ✏️ Text / Lyrics — adds a text block. Plain textarea for lyrics, lines, ideas, descriptions.  
\- 📷 Photo / Image — adds an image block. File picker (or camera on mobile). Multiple images can be added.  
\- 📎 File — adds a file attachment.

Each block appears in the capture area as it's added. Multiple blocks of the same type are fine (two audio recordings, or lyrics \+ notes text). Blocks can be removed individually with an X button.

\*\*Below the content blocks:\*\* Quick metadata fields that are always visible:
...
\- Section header: name (editable), drag handle for reordering (via @dnd-kit)  
...
\- \*\*Production Journal\*\* — Tiptap 3 rich text editor. Organized by topic if the user wants (e.g., "Bass", "Brass", "Mix Notes") or just a running log. Image embeds for gear photos.  
...
\- If MIDI: note list or simple piano-roll-like visualization (horizontal bars on a pitch axis — read-only, not editable). Playback controls. Patch selector to change the playback sound (via smplr/Tone.js).  
...
\*\*Main area: Track listing.\*\* Ordered list of songs. Drag to reorder (via @dnd-kit). Each row shows: track number, song title, status badge, key, tempo. Click to open Song Workspace for that song.

"Add Song" button — create new or select existing.

\*\*Below or sidebar: Album-level content.\*\*

\- \*\*Global Production Notes\*\* — Tiptap 3 rich text editor. Signal chains, gear lists, mixing methodology, sonic direction. This is the album-wide production bible.  
...
\- Format options:  
  \- "Save to Folder" (uses File System Access API — user picks a directory). Only shown if the browser supports showDirectoryPicker (Chrome/Edge).  
  \- "Download as ZIP" — always available as fallback. Uses JSZip \+ browser-fs-access for graceful handling across browsers.  
...
User is noodling on a MIDI keyboard at their desk
  ...
  → Clicks "Record MIDI" to add a MIDI recording block

  → MIDI controller is auto-detected, patch defaults to Piano

  → User selects "Bass" patch (via smplr soundfont)
...
Song is in "production" status, user wants to start tracking in Ableton
  ...
  → User clicks "Save to Folder" (Chrome/Edge), picks \~/Music/The Show/  
    (Or "Download as ZIP" on Firefox/Safari)
